...
e=1.6*10^(-19);
syms v
f_0=N.*(((gamma(0.25)).^4)/(sqrt(2).*(pi.^2)))*((m/(12.*sqrt(2).*pi*k_t)).^(3/2))*exp(-(((gamma(0.25))^4)/(74*pi.^2)).*(m*v^2/(2*k_t))^2);
R_f_0=diff(f_0);
...
for n=1:totalTimeStep
    ...
    if mod(n,100)==0||n==totalTimeStep
        ...
        saveas(h_1,'Eplasmaprime_d_50 ','fig') 
        saveas(h_3,'Hplasmaprime_d_50 ','fig')
        saveas(h_5,'F_1plasmaprime_d_50 ','fig') 
        saveas(h_7,'Jxplasmaprime_d_50 ','fig')        
        save('plasma_prime_all_variable_d_50')
    end
    disp(n)
end
